...
\abstract{Using on-shell diagrams we calculate various forms in $\mathcal{N}=0$ and analyze}
...
\begin{equation}
	...
	\end{aligned}
\end{equation}
Having now showed the general procedure to calculate forms from the on-shell diagrams, we will show how to extend this approach to amplitudes with $\mathbb{N}\neq 4$ super-symmetries. 
\newpage
\section{Calculating $\mathcal{N}<4$ amplitudes}
\subsection{The measure}
Since going to $\mathcal{N}<4$ means that the graphs have to be oriented, one has to consider all internal orientations for specific external helicity configuraions. Further, for diagrams containing closed internal loops, one has to add an extra factor, $\mathcal{J}$ , in
the measure,
\begin{equation}
	\begin{aligned}
...
\begin{equation}
	...
	\end{aligned}
\end{equation}
with $f_i$ denoting the face of a closed cycle, and the sums are over disjoint collections of these closed cycles. One also has to note that each face is defined as \textit{clockwise-oriented product of edge-variables}, meaning that a counterclockwise cycle would produce $\frac{1}{f}$. This is easiest seen through an example, 
\begin{figure}[H]
	\centering
...
To illustrate what the form of the bare\footnote{By bare we mean the form without the internal cycle Jacobian} diagram should look like we start by treating a diagram without an internal cycle.  We take the same diagram as we did in section ...
\begin{figure}[H]
	\centering
...
\end{align*}
We see that sending $\{\alpha_1,\alpha_2,\alpha_4,\alpha_6,\alpha_7,\alpha_8\}\to 0$ erases the corresponding edge, while the same happens by sending $\{\alpha_3,\alpha_5\}\to \infty$. 
...
At four points we can attach the two connected vertices to make the following two-loop diagram, with corresponding C-matrix
\begin{figure}[H]
	\centering
...
In this report we gave a brief introduction to the Grassmanian and on-shell diagrams. In sections --- we outlined the how to get  $\mathcal{N}=4$ sYM amplitudes from these, by explicitly calculating examples at four, five, and six points. We then show how to extend the techniques to $\mathcal{N}\neq4$ by considering all internal configurations for a specific fixed external helicity configuration, specifically focusing on $\mathcal{N}=0$. When diagrams contain an internal cycle, a Jacobian contributes, and the form does not give the amplitudes but rather represent cuts of loop integrands, while the \textit{bare} form, (i.e. the form without the Jacobian), gave the normal MHV Parke-Taylor amplitudes.

We then considered higher loop diagrams at four- and five-point, which had one parameter unfixed. Here  

We then considered the singularity structure
\appendix
