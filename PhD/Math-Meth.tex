...
First we define $\xi\equiv t-i$, then we exandt around the saddle point, assuming that $z$ is large
\begin{equation}
	\begin{aligned}
...
\begin{equation}
	\begin{aligned}
		...
		\int_{0}^{\infty}\dd r\, e^{-\frac{z}{2}r^2}\\
		&=\sqrt{\frac{2}{\pi z}}e^{-\frac{1}{4} \pi i+iz-i\frac{1}{2}\pi\nu}
		\end{aligned}
	\end{equation}
For the Hankel function of the second kind, we had to choose the other maximum. This in turn just changes the sign og the exponentials, since $\theta$ now changes and we so have
\begin{equation}
	\begin{aligned}
		H_\nu^{(1)}(z)
		&=\sqrt{\frac{2}{\pi z}}e^{-\frac{1}{4} \pi i+iz-i\frac{1}{2}\pi\nu}+H.C\\
...
\begin{equation}
 ...
 Z(\beta)=\exp[\sum_{s=1}^{k}\frac{\Gamma(s)\zeta(s+1)A_s}{\beta^{s}}+	D'(0)-D(0)\log \beta]+H.C.
	\end{aligned}
\end{equation}
%It will also be of value later to get the (q'th) derivative of this which is
%\begin{equation}
%	\begin{aligned}
%		\dv[q]{}{\beta}\log Z(\beta)=\frac{1}{(-1)^q}\left[\sum_{s=1}^{k}\frac{\Gamma(s)\zeta(s+1)A_s(z)}{\beta^{s+q}}+	\frac{1}{\beta^{q}}D(0)\Gamma(k)\right]+H.C.
%	\end{aligned}
%\end{equation}
\subsection*{Part d}
