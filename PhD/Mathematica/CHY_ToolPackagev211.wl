...
kk[i_,j_]:=kk @@ Sort[{i,j}] /;  !OrderedQ[{i,j}] &&
      FreeQ[{i,j}, Pattern];
momentumcon1[x_]:=Solve[{Sum[s[i,#],
	{i,Range[x]}]}&/@Range[x]==0][[1]];
momentumcon11[x_]:=Solve[{Sum[kk[i,#],
	{i,Range[x]}]}&/@Range[x]==0][[1]];
momentumcon2[x_]:=Solve[{Sum[ek[#,i],
	{i,Range[x]}]}&/@Range[x]==0][[1]];

(*momentumcon[n] yields the n-point replacement rules for momentum conservation*)
...
matC[N_]:=Table[If[i!= j,ek[i,j]/z[i,j],1]If[i==j&&i==1,Sum[(ek[i,l]z[l,2])/(z[2,1]z[1,l]),{l,3,N}],1]If[i==j&&i>1,Sum[(ek[i,l]z[l,1])/(z[1,i]z[i,l]),{l,DeleteCases[Range[2,N],i]}],1],{i,N},{j,N}];
matPsi[N_]:=matPsi[N]=ArrayFlatten[{{matA[N],-Transpose@matC[N]},{matC[N],matB[N]}}];
defPsi[N_]:=defPsi[N]=Module[{i,j},
Psi={};
row=ConstantArray[{},N];
row2=ConstantArray[{},N];
For[i=1, i<= N,i++,
	For[j=1, j<= N, j++,
		If[i!=j,
			row[[i]]=Append[row[[i]],s[i,j]/((z[i,j]))],
			row[[i]]=Append[row[[i]],0]
			];
		];
	For[j=1, j<= N, j++,
		If[i!=j,
			row[[i]]=Append[row[[i]],ek[j,i]/((z[j,i]))],
			row[[i]]=Append[row[[i]],-Sum[If[i!= c, If[i==1,ek[i,c]*(z[c,2])/((z[2,1])*(z[i,c])),
				ek[i,c]*(z[c,1])/((z[1,i])*(z[i,c]))],0],{c,N}]];
			];
		];
	Psi=Append[Psi,row[[i]]];
	];
For[i=1, i<= N,i++,
	For[j=1, j<= N, j++,
		If[i!=j,
		row2[[i]]=Append[row2[[i]],-ek[i,j]/((z[i,j]))],
		row2[[i]]=Append[row2[[i]],Sum[If[i!= c,If[i==1,ek[i,c]*(z[c,2])/((z[2,1])*(z[i,c])),
		ek[i,c]*(z[c,1])/((z[1,i])*(z[i,c]))],0],{c,N}]];
			];
		];
	For[j=1, j<= N, j++,
		If[i!=j,
		row2[[i]]=Append[row2[[i]],ee[i,j]/((z[i,j]))],
		row2[[i]]=Append[row2[[i]],0]
			];
		];
	Psi=Append[Psi,row2[[i]]];
...
,{j,N}];
,{i,4}];
Print[StringForm["\\begin{polygon}[``]",N]];
Do[Do[
