...
(*Define matrices ABC*)
matA[N_]:=Table[If[i!= j,2*kk[i,j]/z[i,j],0],{i,N},{j,N}];
matB[N_]:=Table[If[i!= j,2*ee[i,j]/z[i,j],0],{i,N},{j,N}];

matC[N_]:=Table[If[i!= j,2*ek[i,j]/z[i,j],1]If[i==j&&i==1,Sum[(ek[i,l]z[l,2])/(z[2,1]z[1,l]),{l,3,N}],1]If[i==j&&i>1,Sum[(ek[i,l]z[l,1])/(z[1,i]z[i,l]),{l,DeleteCases[Range[2,N],i]}],1],{i,N},{j,N}];
